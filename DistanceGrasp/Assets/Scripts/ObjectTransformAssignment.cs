...
//             current_object.transform.rotation = matrix_unity; //rotation_unity;
...
public class ObjectTransformAssignment : MonoBehaviour
{
    ...
    void AssignRandomTransformToGameObjectSet()
    {
        for (int i = 0; i < objects.Length; i++)
        {
            ...
            if (!transformData.ContainsKey(object_name))
            {
                Debug.LogWarning($"No transform data found for {object_name}");
                continue;
            }

            ObjectTransformData object_transform_set = transformData[object_name];

            int randomIndex = UnityEngine.Random.Range(0, object_transform_set.object_rotation.Count);
            List<float> rotationData = object_transform_set.object_rotation[randomIndex];
            
            if (rotationData.Count != 4)
            {
                Debug.LogError($"Invalid rotation data length for {object_name} at index {randomIndex}");
                continue;
            }

            Quaternion rotation_unity = new Quaternion(rotationData[0], rotationData[1], rotationData[2], rotationData[3]);
            current_object.transform.rotation = rotation_unity;
            Debug.Log($"Assigned {object_name} from {object_transform_set.object_rotation[randomIndex]} index {randomIndex} with matrix"  + string.Join(", ", rotation_unity));
